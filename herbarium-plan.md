...
- `link_resolutions.losing_objects` stays empty in the current pass — GNU ld's map file only records what was included, never what was skipped. Recovering losers requires cross-referencing archive contents (`ar t` + nm per member) against the winner. Deferred until a real workflow needs it.
...
**Weak-symbol resolution requires the linker map.** Not all users will add `-Wl,-Map`.
*Mitigation:* herbarium degrades gracefully — `link_resolutions` falls back to `nm`-based inference. Missing map only means losing entries in `losing_objects` and less detail on archive-member selection; winning symbol is still correctly identified. The `describe_link_resolution` response tags rows with the confidence source so the agent knows which is which.
