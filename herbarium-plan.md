...
- Response-size contract: any tool whose result set scales with the project caps its rows (`limit`, default per tool, max 2000) and reports `truncated`, and no tool attaches source snippets unless asked (`include_snippets`). This is a correctness property, not a cosmetic one — an MCP client that hits its output limit truncates the *whole* payload, leaving the caller unable to tell what it lost or that it lost anything, which a self-imposed cap plus an honest flag avoids.
