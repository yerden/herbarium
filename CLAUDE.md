...
- `herbarium serve` — opens an `.hbr` read-only, exposes 31 MCP tools over stdio or streamable HTTP.
...
internal/
  ...
  mcp/                  MCP server + 31 tools; tests build fixture .hbr in-process
testdata/
...
6. `ingest.DWARF` → UPSERT signatures + `decl_file`/`decl_line`; repairs `symbol_definitions.file`/`line` for functions with no `.ci` node (see Known gaps); owns `indirect_call_sites`, `inline_instances` and the **type plane** (`types`, `type_fields`, `enum_constants`); rebuilds `symbols_fts` and `types_fts`. The type plane rides this pass rather than getting its own because `dwarfingest.Read` is the most expensive operation in collect and already runs once per `.o`.
...
- **Types:** `find_type`, `describe_type`. One search covers typedefs, struct/union/enum tags **and** individual enum constants on purpose — an agent hunting an identifier does not know which of those it is, and making it guess is how this search failed before the plane existed. `describe_type` returns field byte offsets, which is the `(struct, offset) → field` fact the removed `.devirt` plane would have carried.
...
- **Types are a separate plane with the same reached-the-assembler caveat.** `types` / `type_fields` / `enum_constants` (v10) come from DWARF, need no new build flag, and are file-scoped so a header type collapses to one row across every including TU. But DWARF records a type only where emitted code *uses* it: a declared-but-unused typedef, struct or enum is absent at `-g` **and** `-g3`. An empty `find_type` therefore means "no TU used this type", and `usedTypesCaveat` in `type_tools.go` is the single place that wording lives. Types outside `--project-root` are skipped, or every TU's `size_t` would bury the project's own.
...
- **`target` means two different planes, and an empty result reads as absence on both.** On `find_symbol` / the linkage tools it joins `link_resolutions` — so an internal-linkage symbol (a `static`, a `static inline` in a header) is filtered out even when its code is in the binary, and "0 hits" means *nothing resolved under that name*, not *no such code*. On the source tools (`list_source_files`, `search_source`, `list_source_drift`) it joins `target_sources` — the TUs meson compiles into that target — so an executable that links a `lib/` archive lists only its `app/` files, and the archive's sources answer under the *library* target's name. Both caveats live on the `target` argument descriptions, centralised in two helpers — `targetSourcesArg` (`source_tools.go`) for the source plane, `targetReachabilityArg` (`location.go`) for the reachability one — because the loss is invisible in the payload: `list_callees` on the fixture's `main` returns 6 callees unscoped and 4 with `target=app1`, dropping the `static` one with no flag, and `list_indirect_call_sites` goes 2 → 0. `TestTargetArgsDiscloseTheirPlane` in `mcp_test.go` fails any tool with a `target` argument that names neither plane, so a new tool has to choose a helper or record a deliberate exemption. The exempt list is the interesting half: the link-plane tools (`describe_link_resolution`, `list_undefined_symbols`, `list_entry_points`, `list_icf_groups`) exclude internal linkage *correctly* — the linker's view is their subject — and the runtime callgraph tools filter on `call_edges.target_id`, which drops nothing. A report of "I read the empty result as 'not linked'" means the wording failed, not the filter.
